...
function acao_set($d,$linkSet,$sysusuSet) {
	...
	$syscliente = mysql_fetch_array(mysql_query("SELECT * FROM syscliente WHERE id='".$d."'"));

	$acao_set = "<div class='btn-group'>";
	
	if(trim($sysperm_acao['editar_syscliente'])==1||trim($sysperm_acao['todos_syscliente'])==1) { 
	
		$acao_set .= "<div style='float:left;width:16px;margin-left:10px;'>";
		$acao_set .= "	<a class=\"img_action_edit popup_fancy ptip_se\" onclick=\"gera_documento_syscliente('".$d."','".$sysusuSet."');\" href=\"javascript:void(0);\" title=\"Gerar Documento\"><img src=\"".$linkSet."template/img/icones_novos/16/contrato_grey.png\" /></a>";
		$acao_set .= "</div>";
	} 
	...
	if(trim($syscliente['criador'])==$sysusuSet||trim($sysperm_acao['todos_syscliente'])==1) { 
	
		if(trim($sysperm_acao['excluir_syscliente'])==1||trim($sysperm_acao['todos_syscliente'])==1) { 
			$acao_set .= "<div style='float:left;width:16px;margin-left:10px;'><a href='javascript:void(0);' onclick=remover_item_tabela('".$d."','syscliente','NAO','') class='btn-mini ptip_se' title='Remover'><img src='".$linkSet."template/img/icones_novos/16/remover-x.png' /></a></div>";
		} 
		
		if(trim($syscliente['stat'])=="1") { 
			if(trim($sysperm_acao['despublicar_syscliente'])==1||trim($sysperm_acao['todos_syscliente'])==1) { 
				$acao_set .= "<div style='float:left;width:16px;margin-left:10px;'><a href='javascript:void(0);' onclick=muda_stat('syscliente','".$d."','0') class='btn-mini ptip_se' title='Despublicar'><img src='".$linkSet."template/img/icones_novos/16/stat-1.png' /></a></div>";
			} else { 
			...
			if(trim($sysperm_acao['publicar_syscliente'])==1||trim($sysperm_acao['todos_syscliente'])==1) { 
				$acao_set .= "<div style='float:left;width:16px;margin-left:10px;'><a href='javascript:void(0);' onclick=muda_stat('syscliente','".$d."','1') class='btn-mini ptip_se' title='Publicar'><img src='".$linkSet."template/img/icones_novos/16/stat-0.png' /></a></div>";
			} else { 
...
function situacao_set($d) {
	...
	$rSqlProcessoTipo = mysql_fetch_array(mysql_query("SELECT * FROM syscliente_tipo WHERE id='".$d."'"));
