...
if(is_dir("../../files/syscliente")) { 
	if(is_dir("../../files/syscliente/".$numeroUnicoGet."")) { 
	} else {
		criaPastaComCaminho("files/syscliente/","".$numeroUnicoGet."");
	}
} else {
	if(is_dir("../../files/syscliente/".$numeroUnicoGet."/")) { 
	} else {
		criaPastaComCaminho("files/","syscliente");
		criaPastaComCaminho("files/syscliente/","".$numeroUnicoGet."");
	}
}
...
if(trim($extensao)=="zip") {
	...
	$caminhoUpload = "../../files/syscliente/".$numeroUnicoGet."/";
	...
	$qtd_fotos = mysql_num_rows(mysql_query("SELECT * FROM syscliente_galeria WHERE numeroUnico='".$numeroUnicoGet."'"));
	...
	for($i=0;$i < $narray; $i++) {
		...
		$data = date("Y-m-d H:i:s");
		$insert = mysql_query("INSERT INTO syscliente_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
															VALUES 
	...
	if(trim($extensao)=="jpg"||trim($extensao)=="jpeg"||trim($extensao)=="gif"||trim($extensao)=="bmp"||trim($extensao)=="png") {
		...
		$tamanhoGet = tamanhoArquivoSemExtensao("../../files/syscliente/".$numeroUnicoGet."/".$arquivo."");
