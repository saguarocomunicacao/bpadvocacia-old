...
if(is_dir("../../files/adv_processo")) { 
	if(is_dir("../../files/adv_processo/".$numeroUnicoGet."")) { 
	} else {
		criaPastaComCaminho("files/adv_processo/","".$numeroUnicoGet."");
	}
} else {
	if(is_dir("../../files/adv_processo/".$numeroUnicoGet."/")) { 
	} else {
		criaPastaComCaminho("files/","adv_processo");
		criaPastaComCaminho("files/adv_processo/","".$numeroUnicoGet."");
	}
}
...
if(trim($extensao)=="zip") {
	...
	$caminhoUpload = "../../files/adv_processo/".$numeroUnicoGet.""; 
	...
	$qtd_fotos = mysql_num_rows(mysql_query("SELECT * FROM adv_processo_galeria WHERE numeroUnico='".$numeroUnicoGet."'"));
	...
	for($i=0;$i < $narray; $i++) {
		...
		$data = date("Y-m-d H:i:s");
		$insert = mysql_query("INSERT INTO adv_processo_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
															VALUES 
	...
	if(trim($extensao)=="jpg"||trim($extensao)=="jpeg"||trim($extensao)=="gif"||trim($extensao)=="bmp"||trim($extensao)=="png") {
		...
		$tamanhoGet = tamanhoArquivoSemExtensao("../../files/adv_processo/".$numeroUnicoGet."/".$arquivo."");
