<div id="view_thumb" style="width:100%;margin-left:-20px;">
    ...
    <div style="width:100%;margin-left:20px;"><div onclick="remover_portfolio_lista('sysfornecedor_galeria');" class="btn"><i class="splashy-remove"></i>&nbsp;&nbsp;Remover selecionados</div></div>
    <ul style="list-style-type:none;margin-left:18px;width:100%;">
        <?
        $qSql = mysql_query("SELECT * FROM sysfornecedor_video WHERE numeroUnico='".$numeroUnicoGet."' ORDER BY ordem");
        ...
        <li style="width:225px;height:130px;float:left;margin-right:10px;margin-bottom:10px;padding:5px;">
            ...
            <div style="float:left;width:225px;height:128px;overflow:hidden;vertical-align:middle;text-align:center;">
                ...
                <a class="img_action_edit popup_fancy" href="<?=$link?>acoes/sysgeral/form-portfolio.php?idS=<?=$rSql['id']?>&modS=sysfornecedor_video" title="Editando <?=$rSql['nome']?>"><i class="splashy-pencil"></i></a>
                <a href="javascript:void(0);" onclick="remover_portfolio_unico('<?=$rSql['id']?>','sysfornecedor_galeria');" class="img_action_remove" title="Remover"><i class="splashy-remove"></i></a>
            </div>
        </li>
        <? } ?>
    </ul>
