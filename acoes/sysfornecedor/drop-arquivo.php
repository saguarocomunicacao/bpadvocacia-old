...
if(is_dir("../../files/sysfornecedor")) { 
	if(is_dir("../../files/sysfornecedor/".$numeroUnicoGet."")) { 
	} else {
		criaPastaComCaminho("files/sysfornecedor/","".$numeroUnicoGet."");
	}
} else {
	if(is_dir("../../files/sysfornecedor/".$numeroUnicoGet."/")) { 
	} else {
		criaPastaComCaminho("files/","sysfornecedor");
		criaPastaComCaminho("files/sysfornecedor/","".$numeroUnicoGet."");
	}
}
...
if(trim($extensao)=="zip") {
	...
	$caminhoUpload = "../../files/sysfornecedor/".$numeroUnicoGet."/";
	...
	$qtd_fotos = mysql_num_rows(mysql_query("SELECT * FROM sysfornecedor_galeria WHERE numeroUnico='".$numeroUnicoGet."'"));
	...
	for($i=0;$i < $narray; $i++) {
		...
		$data = date("Y-m-d H:i:s");
		$insert = mysql_query("INSERT INTO sysfornecedor_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
															VALUES 
	...
	if(trim($extensao)=="jpg"||trim($extensao)=="jpeg"||trim($extensao)=="gif"||trim($extensao)=="bmp"||trim($extensao)=="png") {
		...
		$tamanhoGet = tamanhoArquivoSemExtensao("../../files/sysfornecedor/".$numeroUnicoGet."/".$arquivo."");
