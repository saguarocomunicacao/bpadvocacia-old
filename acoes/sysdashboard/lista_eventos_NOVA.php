...
if(trim($filtroDataGet)=="") { } else {
	$filtroDataSet = " a.data BETWEEN '".$filtroDataGet_ini." 00:00:00' AND '".$filtroDataGet_fim." 23:59:59' ";
}

$sysusu_set = mysql_fetch_array(mysql_query("SELECT cor FROM sysusu WHERE id='".$criadorGet."'"));
...
";
$qSql = mysql_query("".$strSQL." ".$filtroData_set." ORDER BY a.data_inicio, a.hora_inicio");
...
LEFT JOIN sysagenda_categoria AS b ON (b.id = a.idsysagenda_categoria)
...
LEFT JOIN adv_processo_tipo AS b ON (b.id = a.idadv_processo_tipo)
...
while($rSql = mysql_fetch_array($qSql)) {
	...
	if(trim($adv_processo_tipo_set['cor'])=="") { $corSet = "#333333"; } else { $corSet = "".$rSql['cor'].""; }
...
LEFT JOIN adv_intimacao AS b ON (b.numeroUnico = a.numeroUnico_pai)

";
$qSql = mysql_query("".$strSQL." ".$filtroData_set." ORDER BY a.data_fim, a.hora_fim");
while($rSql = mysql_fetch_array($qSql)) {
