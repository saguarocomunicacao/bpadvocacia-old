...
if(is_dir("../../files/foto")) { 
	criaPastaComCaminho("files/foto/","".$numeroUnicoGet."");
} else {
	if(is_dir("../../files/foto/".$numeroUnicoGet."")) { 
	} else {
		criaPastaComCaminho("files/","foto");
		criaPastaComCaminho("files/foto/","".$numeroUnicoGet."");
	}
}
...
if(trim($extensao)=="zip") {
	...
	$caminhoUpload = "../../files/foto/".$numeroUnicoGet."/";
	...
	$qtd_fotos = mysql_num_rows(mysql_query("SELECT * FROM foto_galeria WHERE numeroUnico='".$numeroUnicoGet."'"));
	...
	for($i=0;$i < $narray; $i++) {
		...
		$data = date("Y-m-d H:i:s");
		$insert = mysql_query("INSERT INTO foto_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
															VALUES 
	...
	if(trim($extensao)=="jpg"||trim($extensao)=="jpeg"||trim($extensao)=="gif"||trim($extensao)=="bmp"||trim($extensao)=="png") {
		...
		$tamanhoGet = tamanhoArquivoSemExtensao("../../files/foto/".$numeroUnicoGet."/".$arquivo."");
