    ...
    <tbody>
        ...
        while($rSql = mysql_fetch_array($qSql)) {
			...
			if($nSql==0) {
				...
				if($rSqlPerm['excluir_arquivo']==0&&$rSqlPerm['renomear_arquivo']==0&&$rSqlPerm['baixar_arquivo']==0) {
					$mostra = 0;
				} else {
					$mostra = 1;
				}
		...
		if($mostra==1||$rSql['idsysusu']==$idsysusuGet) {
        ...
        <tr>
            ...
            <td>
                ...
                <? } else { ?>
				<? if($rSqlPerm['renomear_pasta']==0) { } else { ?><a class="img_action_edit popup_fancy" href="<?=$link?>acoes/sysmidia/form-pasta.php?idS=<?=$rSql['id']?>&idsysusuS=<?=$idsysusuGet?>" title="Editar"><i class="splashy-pencil"></i></a><? } ?>
                <? if($rSqlPerm['excluir_pasta']==0) { } else { ?><a class="img_action_remove" onclick="remover_pasta('<?=$rSql['id']?>');" href="javascript:void(0)" title="Remover"><i class="splashy-remove"></i></a><? } ?>
                <? } ?>
            </td>
        </tr>
        <? } } ?>
        ...
        <tr>
            ...
            <td>
                ...
                <? } else { ?>
                <? if($rSqlPerm['baixar_arquivo']==0) { } else { ?><a href="<?=$link?>include/lib/forca-download.php?arquivo=../../files/sysmidia/<?=$rSql['numeroUnico']?>/<?=$rSql['nome']?>"><i class="splashy-zoom_in"></i></a><? } ?>
                <? } ?>
                <? } ?>
                <? if($idsysusuGet==$rSql['idsysusu']) { ?>
                ...
                <? } else { ?>
                <? if($rSqlPerm['renomear_arquivo']==0) { } else { ?><a class="img_action_edit popup_fancy" href="<?=$link?>acoes/sysmidia/form-arquivo.php?idS=<?=$rSql['id']?>&idsysusuS=<?=$idsysusuGet?>" title="Editar"><i class="splashy-pencil"></i></a><? } ?>
                <? if($rSqlPerm['excluir_arquivo']==0) { } else { ?><a class="img_action_remove" href="javascript:void(0)" onclick="remover_arquivo('<?=$rSql['id']?>');" title="Remover"><i class="splashy-remove"></i></a><? } ?>
                <? if($rSqlPerm['baixar_arquivo']==0) { } else { ?><a class="img_action_remove" href="<?=$link?>include/lib/forca-download.php?arquivo=../../files/sysmidia/<?=$rSql['numeroUnico']?>/<?=$rSql['nome']?>" title="Faça o download deste arquivo"><i class="splashy-download"></i></a><? } ?>
                <? } ?>
            </td>
        </tr>
        <? } } ?>
