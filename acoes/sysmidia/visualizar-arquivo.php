...
<img style="max-width:600px;" src="<?=$link?>files/sysmidia/<?=$itemSql['numeroUnico']?>/<?=$itemSql['arquivo']?>" alt="" />
<? } else { ?>
    ...
    <script type="text/javascript">
        jwplayer("myElement").setup({
            file: "<?=$link?>files/sysmidia/<?=$itemSql['numeroUnico']?>/<?=$itemSql['arquivo']?>",
			width:450,
			heigth:350
        });
    </script>
    <? } else { ?>
        ...
        <param name="fileName" value="<?=$link?>files/sysmidia/<?=$itemSql['numeroUnico']?>/<?=$itemSql['arquivo']?>">
        <param name="animationatStart" value="true">
        <param name="transparentatStart" value="true">
        <param name="autoStart" value="true">
        <param name="showControls" value="true">
        <param name="Volume" value="-450">
        <embed type="application/x-mplayer2" pluginspage="http://www.microsoft.com/Windows/MediaPlayer/" src="<?=$link?>files/sysmidia/<?=$itemSql['numeroUnico']?>/<?=$itemSql['arquivo']?>" name="MediaPlayer1" width="450" height="350" autostart="0" showcontrols="1" volume="-450">
        </object>
		<? } else { ?>
			<? if($extensao=="doc"||$extensao=="docx") { ?>
                <?
					...
					header("Content-Disposition: attachment; filename=".$link."files/sysmidia/".$itemSql['numeroUnico']."/".$itemSql['arquivo'].";");
				?>
			<? } else { ?>
            <? } ?>
        <? } ?>
