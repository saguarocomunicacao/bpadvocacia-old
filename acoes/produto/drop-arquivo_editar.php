...
if(is_dir("../../files/sysproduto")) { 
	criaPastaComCaminho("files/sysproduto/","".$numeroUnicoGet."");
} else {
	if(is_dir("../../files/sysproduto/".$numeroUnicoGet."")) { 
	} else {
		criaPastaComCaminho("files/","sysproduto");
		criaPastaComCaminho("files/sysproduto/","".$numeroUnicoGet."");
	}
}
...
if(trim($extensao)=="zip") {
	...
	$caminhoUpload = "../../files/sysproduto/".$numeroUnicoGet."/";
	...
	$qtd_fotos = mysql_num_rows(mysql_query("SELECT * FROM sysproduto_galeria WHERE numeroUnico='".$numeroUnicoGet."'"));
	...
	for($i=0;$i < $narray; $i++) {
		...
		$data = date("Y-m-d H:i:s");
		$insert = mysql_query("INSERT INTO sysproduto_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
															VALUES 
	...
	if(trim($extensao)=="jpg"||trim($extensao)=="jpeg"||trim($extensao)=="gif"||trim($extensao)=="bmp"||trim($extensao)=="png") {
		...
		$tamanhoGet = tamanhoArquivoSemExtensao("../../files/sysproduto/".$numeroUnicoGet."/".$arquivo."");
