...
$numeroUnicoGet = $_SESSION["numeroUnico_upload_arquivo_editar"];

if(is_dir("../../files/systemplate")) { 
	criaPastaComCaminho("files/systemplate/","".$numeroUnicoGet."");
} else {
	if(is_dir("../../files/systemplate/".$numeroUnicoGet."")) { 
	} else {
		criaPastaComCaminho("files/","systemplate");
		criaPastaComCaminho("files/systemplate/","".$numeroUnicoGet."");
	}
}
...
if(trim($extensao)=="zip") {
	...
	$caminhoUpload = "../../files/systemplate/".$numeroUnicoGet."/";
	...
	$qtd_fotos = mysql_num_rows(mysql_query("SELECT * FROM systemplate_galeria WHERE numeroUnico='".$numeroUnicoGet."'"));
	...
	for($i=0;$i < $narray; $i++) {
		...
		$data = date("Y-m-d H:i:s");
		$insert = mysql_query("INSERT INTO systemplate_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
															VALUES 
	...
	if(trim($extensao)=="jpg"||trim($extensao)=="jpeg"||trim($extensao)=="gif"||trim($extensao)=="bmp"||trim($extensao)=="png") {
		...
		$tamanhoGet = tamanhoArquivoSemExtensao("../../files/systemplate/".$numeroUnicoGet."/".$arquivo."");
