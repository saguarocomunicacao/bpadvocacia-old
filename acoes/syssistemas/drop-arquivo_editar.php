...
if(is_dir("../../files/syssistemas")) { 
	criaPastaComCaminho("files/syssistemas/","".$numeroUnicoGet."");
} else {
	if(is_dir("../../files/syssistemas/".$numeroUnicoGet."")) { 
	} else {
		criaPastaComCaminho("files/","syssistemas");
		criaPastaComCaminho("files/syssistemas/","".$numeroUnicoGet."");
	}
}
...
if(trim($extensao)=="zip") {
	...
	$caminhoUpload = "../../files/syssistemas/".$numeroUnicoGet."/";
	...
	$qtd_fotos = mysql_num_rows(mysql_query("SELECT * FROM syssistemas_galeria WHERE numeroUnico='".$numeroUnicoGet."'"));
	...
	for($i=0;$i < $narray; $i++) {
		...
		$data = date("Y-m-d H:i:s");
		$insert = mysql_query("INSERT INTO syssistemas_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
															VALUES 
	...
	if(trim($extensao)=="jpg"||trim($extensao)=="jpeg"||trim($extensao)=="gif"||trim($extensao)=="bmp"||trim($extensao)=="png") {
		...
		$tamanhoGet = tamanhoArquivoSemExtensao("../../files/syssistemas/".$numeroUnicoGet."/".$arquivo."");
