...
?>
                                    <? $syscliente_set = mysql_fetch_array(mysql_query("SELECT * FROM syscliente WHERE id='".$_REQUEST['idSysclienteS']."'")); ?>
									...
									<?
                                    $qSqlProcesso = mysql_query("SELECT * FROM adv_processo_syscliente WHERE idsyscliente='".$syscliente_set['id']."' ORDER BY data DESC");
                                    while($rSqlProcesso = mysql_fetch_array($qSqlProcesso)) {
                                        $processo_set = mysql_fetch_array(mysql_query("SELECT * FROM adv_processo WHERE numeroUnico='".$rSqlProcesso['numeroUnico_pai']."'"));
                                    ?>
                                    <div class="formSep" style="width:710px;">
                                        ...
                                        <div style="float:left;margin-right:10px;width:710px;padding-bottom:5px;">
                                            ...
                                            <div style="float:left;margin-right:10px;">
                                                <label class="req">Contrato</label>
                                                <select id="syscontrato_modelo_<?=$processo_set['id']?>" style="width:300px;">
                                                    <option value="">---</option>
                                                    <?
                                                    $qSqlItem = mysql_query("SELECT * FROM syscontrato_modelo WHERE stat='1' ORDER BY nome");
                                                    while($rSqlItem = mysql_fetch_array($qSqlItem)) {
                                            ...
                                            <div id="btn-gerador_<?=$processo_set['id']?>" style="float:left;margin-top:25px;">
                                                <button type="button" onclick="adv_processo_gera_contrato_cliente('<?=$processo_set['id']?>','<?=$syscliente_set['id']?>','<?=$_REQUEST['idUsuarioS']?>');" class="btn btn-success">Gerar Contrato</button>
                                            </div>
