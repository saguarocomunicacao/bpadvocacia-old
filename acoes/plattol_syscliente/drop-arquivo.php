...
if(is_dir("../../files/plattol_syscliente")) { 
	if(is_dir("../../files/plattol_syscliente/".$numeroUnicoGet."")) { 
	} else {
		criaPastaComCaminho("files/plattol_syscliente/","".$numeroUnicoGet."");
	}
} else {
	if(is_dir("../../files/plattol_syscliente/".$numeroUnicoGet."/")) { 
	} else {
		criaPastaComCaminho("files/","plattol_syscliente");
		criaPastaComCaminho("files/plattol_syscliente/","".$numeroUnicoGet."");
	}
}
...
if(trim($extensao)=="zip") {
	...
	$caminhoUpload = "../../files/plattol_syscliente/".$numeroUnicoGet."/";
	...
	$qtd_fotos = mysql_num_rows(mysql_query("SELECT * FROM plattol_syscliente_galeria WHERE numeroUnico='".$numeroUnicoGet."'"));
	...
	for($i=0;$i < $narray; $i++) {
		...
		$data = date("Y-m-d H:i:s");
		$insert = mysql_query("INSERT INTO plattol_syscliente_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
															VALUES 
	...
	if(trim($extensao)=="jpg"||trim($extensao)=="jpeg"||trim($extensao)=="gif"||trim($extensao)=="bmp"||trim($extensao)=="png") {
		...
		$tamanhoGet = tamanhoArquivoSemExtensao("../../files/plattol_syscliente/".$numeroUnicoGet."/".$arquivo."");
