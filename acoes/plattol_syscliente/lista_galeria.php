<div id="view_thumb" style="float:left;width:100%;margin-left:-20px;">
    ...
    <div style="float:left;width:100%;margin-left:20px;"><div onclick="remover_foto_lista('plattol_syscliente_galeria');" class="btn"><i class="splashy-remove"></i>&nbsp;&nbsp;Remover selecionados</div></div>
    <ul style="list-style-type:none;margin-left:18px;">
        <?
        $qSql = mysql_query("SELECT * FROM plattol_syscliente_galeria WHERE numeroUnico='".$numeroUnicoGet."' ORDER BY ordem");
        ...
        <li style="width:128px;float:left;margin-right:10px;margin-bottom:10px;padding:5px;">
            ...
            <img style="max-width:128px;" src="<?=$link?>files/plattol_syscliente/<?=$rSql['numeroUnico']?>/<?=$rSql['imagem']?>" alt="" />
            </div>
            <? } else { ?>
            ...
            <div class="view_size" style="width:128px;background-color:#EBEBEB;overflow:hidden;font-size:11px;">&nbsp;<? tamanhoArquivo("../../files/plattol_syscliente/".$rSql['numeroUnico']."/".$rSql['imagem'].""); ?></div>
            <div style="float:left;width:128px;height:128px;overflow:hidden;vertical-align:middle;text-align:center;">
                ...
                <a class="img_action_view popup_fancy" href="<?=$link?>acoes/plattol_syscliente/visualizar-arquivo.php?idS=<?=$rSql['id']?>"><i class="splashy-zoom_in"></i></a>
                <? } else { ?>
                <a href="<?=$link?>include/lib/forca-download.php?arquivo=../../files/plattol_syscliente/<?=$rSql['numeroUnico']?>/<?=$rSql['imagem']?>"><i class="splashy-zoom_in"></i></a>
                <? } ?>
                <a class="img_action_edit popup_fancy" href="<?=$link?>acoes/sysgeral/form-foto.php?idS=<?=$rSql['id']?>&modS=plattol_syscliente_galeria" title="Editando <?=$rSql['nome']?>"><i class="splashy-pencil"></i></a>
                <a href="javascript:void(0);" onclick="remover_foto_unico('<?=$rSql['id']?>','plattol_syscliente_galeria');" class="img_action_remove" title="Remover"><i class="splashy-remove"></i></a>
            </div>
        </li>
        <? } ?>
    </ul>
