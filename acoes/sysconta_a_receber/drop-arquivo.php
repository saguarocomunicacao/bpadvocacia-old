...
if(is_dir("../../files/sysconta_a_receber")) { 
	if(is_dir("../../files/sysconta_a_receber/".$numeroUnicoGet."")) { 
	} else {
		criaPastaComCaminho("files/sysconta_a_receber/","".$numeroUnicoGet."");
	}
} else {
	if(is_dir("../../files/sysconta_a_receber/".$numeroUnicoGet."/")) { 
	} else {
		criaPastaComCaminho("files/","sysconta_a_receber");
		criaPastaComCaminho("files/sysconta_a_receber/","".$numeroUnicoGet."");
	}
}
...
if(trim($extensao)=="zip") {
	...
	$caminhoUpload = "../../files/sysconta_a_receber/".$numeroUnicoGet."/";
	...
	$qtd_fotos = mysql_num_rows(mysql_query("SELECT * FROM sysconta_a_receber_galeria WHERE numeroUnico='".$numeroUnicoGet."'"));
	...
	for($i=0;$i < $narray; $i++) {
		...
		$data = date("Y-m-d H:i:s");
		$insert = mysql_query("INSERT INTO sysconta_a_receber_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
															VALUES 
	...
	if(trim($extensao)=="jpg"||trim($extensao)=="jpeg"||trim($extensao)=="gif"||trim($extensao)=="bmp"||trim($extensao)=="png") {
		...
		$tamanhoGet = tamanhoArquivoSemExtensao("../../files/sysconta_a_receber/".$numeroUnicoGet."/".$arquivo."");
