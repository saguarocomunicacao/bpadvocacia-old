...
if(is_dir("../../files/sysagenda")) { 
	if(is_dir("../../files/sysagenda/".$numeroUnicoGet."")) { 
	} else {
		criaPastaComCaminho("files/sysagenda/","".$numeroUnicoGet."");
	}
} else {
	if(is_dir("../../files/sysagenda/".$numeroUnicoGet."/")) { 
	} else {
		criaPastaComCaminho("files/","sysagenda");
		criaPastaComCaminho("files/sysagenda/","".$numeroUnicoGet."");
	}
}
...
if(trim($extensao)=="zip") {
	...
	$caminhoUpload = "../../files/sysagenda/".$numeroUnicoGet.""; 
	...
	$qtd_fotos = mysql_num_rows(mysql_query("SELECT * FROM sysagenda_galeria WHERE numeroUnico='".$numeroUnicoGet."'"));
	...
	for($i=0;$i < $narray; $i++) {
		...
		$data = date("Y-m-d H:i:s");
		$insert = mysql_query("INSERT INTO sysagenda_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
															VALUES 
	...
	if(trim($extensao)=="jpg"||trim($extensao)=="jpeg"||trim($extensao)=="gif"||trim($extensao)=="bmp"||trim($extensao)=="png") {
		...
		$tamanhoGet = tamanhoArquivoSemExtensao("../../files/sysagenda/".$numeroUnicoGet."/".$arquivo."");
