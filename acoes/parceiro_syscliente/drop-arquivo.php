...
if(is_dir("../../files/parceiro_syscliente")) { 
	if(is_dir("../../files/parceiro_syscliente/".$numeroUnicoGet."")) { 
	} else {
		criaPastaComCaminho("files/parceiro_syscliente/","".$numeroUnicoGet."");
	}
} else {
	if(is_dir("../../files/parceiro_syscliente/".$numeroUnicoGet."/")) { 
	} else {
		criaPastaComCaminho("files/","parceiro_syscliente");
		criaPastaComCaminho("files/parceiro_syscliente/","".$numeroUnicoGet."");
	}
}
...
if(trim($extensao)=="zip") {
	...
	$caminhoUpload = "../../files/parceiro_syscliente/".$numeroUnicoGet."/";
	...
	$qtd_fotos = mysql_num_rows(mysql_query("SELECT * FROM parceiro_syscliente_galeria WHERE numeroUnico='".$numeroUnicoGet."'"));
	...
	for($i=0;$i < $narray; $i++) {
		...
		$data = date("Y-m-d H:i:s");
		$insert = mysql_query("INSERT INTO parceiro_syscliente_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
															VALUES 
	...
	if(trim($extensao)=="jpg"||trim($extensao)=="jpeg"||trim($extensao)=="gif"||trim($extensao)=="bmp"||trim($extensao)=="png") {
		...
		$tamanhoGet = tamanhoArquivoSemExtensao("../../files/parceiro_syscliente/".$numeroUnicoGet."/".$arquivo."");
	...
	*/
	...
	$insert = mysql_query("INSERT INTO parceiro_syscliente_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
														VALUES 
													   ('".$numeroUnicoGet."','".$ordem."','sem legenda','".$arquivo."','1','".$data."','".$data."')");
