...
if(is_dir("../../files/portfolio")) { 
	criaPastaComCaminho("files/portfolio/","".$numeroUnicoGet."");
} else {
	if(is_dir("../../files/portfolio/".$numeroUnicoGet."")) { 
	} else {
		criaPastaComCaminho("files/","portfolio");
		criaPastaComCaminho("files/portfolio/","".$numeroUnicoGet."");
	}
}
...
if(trim($extensao)=="zip") {
	...
	$caminhoUpload = "../../files/portfolio/".$numeroUnicoGet."/";
	...
	$qtd_fotos = mysql_num_rows(mysql_query("SELECT * FROM portfolio_galeria WHERE numeroUnico='".$numeroUnicoGet."'"));
	...
	for($i=0;$i < $narray; $i++) {
		...
		$data = date("Y-m-d H:i:s");
		$insert = mysql_query("INSERT INTO portfolio_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
															VALUES 
	...
	if(trim($extensao)=="jpg"||trim($extensao)=="jpeg"||trim($extensao)=="gif"||trim($extensao)=="bmp"||trim($extensao)=="png") {
		...
		$tamanhoGet = tamanhoArquivoSemExtensao("../../files/portfolio/".$numeroUnicoGet."/".$arquivo."");
