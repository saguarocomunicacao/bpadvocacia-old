...
if(is_dir("../../files/sysconta_a_pagar")) { 
	if(is_dir("../../files/sysconta_a_pagar/".$numeroUnicoGet."")) { 
	} else {
		criaPastaComCaminho("files/sysconta_a_pagar/","".$numeroUnicoGet."");
	}
} else {
	if(is_dir("../../files/sysconta_a_pagar/".$numeroUnicoGet."/")) { 
	} else {
		criaPastaComCaminho("files/","sysconta_a_pagar");
		criaPastaComCaminho("files/sysconta_a_pagar/","".$numeroUnicoGet."");
	}
}
...
if(trim($extensao)=="zip") {
	...
	$caminhoUpload = "../../files/sysconta_a_pagar/".$numeroUnicoGet."/";
	...
	$qtd_fotos = mysql_num_rows(mysql_query("SELECT * FROM sysconta_a_pagar_galeria WHERE numeroUnico='".$numeroUnicoGet."'"));
	...
	for($i=0;$i < $narray; $i++) {
		...
		$data = date("Y-m-d H:i:s");
		$insert = mysql_query("INSERT INTO sysconta_a_pagar_galeria (numeroUnico,ordem,nome,imagem,stat,data,dataModificacao) 
															VALUES 
	...
	if(trim($extensao)=="jpg"||trim($extensao)=="jpeg"||trim($extensao)=="gif"||trim($extensao)=="bmp"||trim($extensao)=="png") {
		...
		$tamanhoGet = tamanhoArquivoSemExtensao("../../files/sysconta_a_pagar/".$numeroUnicoGet."/".$arquivo."");
