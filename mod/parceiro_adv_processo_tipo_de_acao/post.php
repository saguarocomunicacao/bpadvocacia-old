<?
	...
	if(trim($_POST['acaoForm'])=="add"||trim($_POST['acaoForm'])=="add-continuar") {
		...
		if(trim($_POST['acaoForm'])=="add-continuar"&&$itemN>0) {
			...
			$qall = mysql_query("SELECT * FROM parceiro_adv_processo WHERE idparceiro_adv_processo_tipo_de_acao='".$idEditavel."' ");
			while($rall = mysql_fetch_array($qall)) {
				$update = mysql_query("UPDATE parceiro_adv_processo SET idparceiro_adv_processo_tipo_de_acao_txt='".$_POST['nome']."' WHERE id='".$rall['id']."'");
			}
					...
					if(trim($_POST['acaoForm'])=="despublicar") {
						...
						if(trim($_POST['acaoForm'])=="editar-permissoes"||trim($_POST['acaoForm'])=="add-continuar_formulario_permissoes") {
							...
							while($rSqlTipo = mysql_fetch_array($qSqlTipo)) {
								$nTipo = mysql_num_rows(mysql_query("SELECT * FROM parceiro_adv_parceiro WHERE idparceiro_adv_processo_tipo_de_acao='".$_POST['iditem']."' AND idsysusu='".$rSqlTipo['id']."'"));
								if($nTipo==0) {
									...
									$item = mysql_fetch_array(mysql_query("SELECT * FROM parceiro_adv_parceiro WHERE idparceiro_adv_processo_tipo_de_acao='".$_POST['iditem']."' AND idsysusu='".$rSqlTipo['id']."'"));
