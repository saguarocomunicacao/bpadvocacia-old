<?
	...
	if(trim($_POST['acaoForm'])=="add"||trim($_POST['acaoForm'])=="add-continuar") {
		...
		if(trim($_POST['acaoForm'])=="add-tarefas") {
			...
			$qall = mysql_query("SELECT * FROM parceiro_syscronograma_item WHERE numeroUnico_pai='".$_POST['numeroUnico_pai']."'");
			while($rall = mysql_fetch_array($qall)) {
				if( $rall['ordem'] >= $_POST['ordem']) {
					$ordem = $rall['ordem'] + 1;
					$update = mysql_query("UPDATE parceiro_syscronograma_item SET ordem='".$ordem."' WHERE id='".$rall['id']."'");
				}
			}
	
			upload_arquivo("parceiro_syscronograma_item","imagem","");
			...
			insert($_POST,"parceiro_syscronograma_item");
		} else {
			if(trim($_POST['acaoForm'])=="editar-tarefas") {
				$mod = "parceiro_syscronograma_item";
	...
	if(trim($_POST['acaoForm'])=="add-tarefas"||trim($_POST['acaoForm'])=="editar-tarefas") {
		$itemPai = mysql_fetch_array(mysql_query("SELECT * FROM parceiro_syscronograma WHERE numeroUnico='".$_POST['numeroUnico_pai']."'"));
