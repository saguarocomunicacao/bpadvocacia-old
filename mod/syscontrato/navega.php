<?
	if(trim($_REQUEST['var3'])=="")    { 
		...
		if(trim($_REQUEST['var3'])=="editar")    { 
			$pagina="mod/".$mod."/list.php";    
			$id = "".$_REQUEST['var4']."";
			$confere = mysql_num_rows(mysql_query("SELECT * FROM ".$linguagem_set."".$mod." WHERE id='".$id."'"));
			if($confere==0) {
				$r = mysql_fetch_array(mysql_query("SELECT * FROM syssistema"));
				$pagina="404.php";    
				$caminho1 = "Página não encontrada"; 
			} else {
				$row = mysql_fetch_array(mysql_query("SELECT * FROM ".$linguagem_set."".$mod." WHERE id='".$id."'"));
				$caminho2 = "Detalhes - ".$row['nome']."";
			}
		} else {
			if(trim($_REQUEST['var3'])=="aceito")    { 
				$pagina="mod/".$mod."/list.php";    
				$id = "".$_REQUEST['var4']."";
				$confere = mysql_num_rows(mysql_query("SELECT * FROM ".$linguagem_set."".$mod." WHERE numeroUnico='".$id."'"));
				if($confere==0) {
					$r = mysql_fetch_array(mysql_query("SELECT * FROM syssistema"));
					$pagina="404.php";    
					$caminho1 = "Página não encontrada"; 
				} else {
					$row = mysql_fetch_array(mysql_query("SELECT * FROM ".$linguagem_set."".$mod." WHERE numeroUnico='".$id."'"));
					$update = mysql_query("UPDATE ".$linguagem_set."".$mod." SET aceito='1',data_aceito='".$data."',dataModificacao='".$data."' WHERE id='".$row['id']."'");
				}
			} else {
