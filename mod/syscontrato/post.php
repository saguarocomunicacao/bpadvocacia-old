<?
	...
	if(trim($_POST['acaoForm'])=="add") {
		...
		$_POST['data_aceito'] = normalTOdate($_POST['data_aceito']);
		...
		if(trim($_POST['acaoForm'])=="editar") {
			...
			if(trim($_POST['acaoForm'])=="excluir") {
				foreach ($_POST['msg_sel'] as $idcheck) {
					...
					$update = mysql_query("UPDATE sysprospecto SET contrato='0',dataModificacao='".$data."' WHERE id='".$itemAtual['idsysprospecto']."'");
