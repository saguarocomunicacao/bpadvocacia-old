<?
	...
	if(trim($_POST['acaoForm'])=="add") {
		...
		upload_arquivo($mod."_item","imagem_capa","");
		upload_arquivo($mod."_item","imagem_interna","");
		...
		insert($_POST,$mod."_item");
	} else {
		if(trim($_POST['acaoForm'])=="editar") {
			$idEditavel = $_POST['iditem'];
			$item = mysql_fetch_array(mysql_query("SELECT * FROM ".$mod."_item WHERE id='".$idEditavel."'"));
			...
			# Gravação do Log
			$itemAntes = mysql_fetch_array(mysql_query("SELECT * FROM ".$mod."_item WHERE id='".$idEditavel."'"));
			$logPerfil = "administrador";
			$logId = $sysusu['id'];
			$logAcao = "Editou";
			$logLocal = "".$caminho1."";
			...
			update($_POST,$mod."_item",$idEditavel);
		} else {
			if(trim($_POST['acaoForm'])=="config") {
				...
				if(trim($_POST['acaoForm'])=="excluir") {
					...
					if(trim($_POST['acaoForm'])=="publicar") {
						foreach ($_POST['msg_sel'] as $idcheck) {
							$sql = mysql_query("UPDATE ".$mod."_item SET stat='1' WHERE id='".$idcheck."'");
						}
					} else {
						if(trim($_POST['acaoForm'])=="despublicar") {
							foreach ($_POST['msg_sel'] as $idcheck) {
								$sql = mysql_query("UPDATE ".$mod."_item SET stat='0' WHERE id='".$idcheck."'");
							}
						} else {
