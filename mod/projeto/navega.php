<?
	if(trim($_REQUEST['var3'])=="")    { 
		...
		if(trim($_REQUEST['var3'])=="editar")    { 
			$pagina="mod/".$mod."/list.php";    
			$id = "".$_REQUEST['var4']."";
			$confere = mysql_num_rows(mysql_query("SELECT * FROM ".$linguagem_set."".$mod."_item WHERE id='".$id."'"));
			if($confere==0) {
				$r = mysql_fetch_array(mysql_query("SELECT * FROM syssistema"));
				$pagina="404.php";    
				$caminho1 = "Página não encontrada"; 
			} else {
				$row = mysql_fetch_array(mysql_query("SELECT * FROM ".$linguagem_set."".$mod."_item WHERE id='".$id."'"));
