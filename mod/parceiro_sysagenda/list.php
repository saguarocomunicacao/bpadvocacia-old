        <!-- main content -->
													...
													if($('#calendar_json').length) {
														$('#calendar_json').fullCalendar({
															...
															eventDrop: function(event,dayDelta,minuteDelta,allDay,revertFunc) {
																atualiza_dia_parceiro_sysagenda(event.id,event.description,dayDelta,minuteDelta,'drop')
															},
															eventResize: function(event,dayDelta,minuteDelta,revertFunc) {
																atualiza_dia_parceiro_sysagenda(event.id,event.description,dayDelta,minuteDelta,'resize')
															}
														});
													}
												}
                                                            ...
                                                            <div class="formSep">
                                                                <div style="float:left;margin-right:10px;padding-bottom:10px;width:550px;">
                                                                    ...
                                                                    <select id="lista_admin_itens" multiple="multiple" >
                                                                        <?
                                                                        $qSqlItem = mysql_query("SELECT * FROM sysusu WHERE stat='1' ORDER BY nome");
                                                                        while($rSqlItem = mysql_fetch_array($qSqlItem)) {
                                                                        ?>
                                                ...
                                                <div id="tb1_lista" class="tab-pane <? if(trim($_REQUEST['var3'])=="") { ?>active<? } ?>">
                                                    ...
                                                    <div class="span2">
                                                        <div class="w-box w-box">
                                                            <div class="w-box-header">Categorias</div>
                                                            <div class="w-box-content">

																<?
                                                                $qSqlItem = mysql_query("SELECT * FROM ".$mod."_categoria WHERE stat='1' ORDER BY ordem");
                                                                while($rSqlItem = mysql_fetch_array($qSqlItem)) {
																	if(trim($lista_categorias)=="") {
																		$lista_categorias = "|".$rSqlItem['id']."|";
																	} else {
																		$lista_categorias = $lista_categorias."|".$rSqlItem['id']."|";
																	}
                                                                ?>
                                                                <div style="width:100%;float:left;margin-top:5px;cursor:pointer;" onclick="filtra_parceiro_sysagenda('<?= $rSqlItem['id'] ?>','<?=$sysusu['id']?>','<?=$_REQUEST['var1']?>','<?=$_REQUEST['var2']?>','<?= $rSqlItem['cor'] ?>','1');">
                                                                    <input id="check-<?= $rSqlItem['id'] ?>" type="checkbox" style="display:none;" checked="checked" value="1" />
                                                                    <div id="div-cor-<?= $rSqlItem['id'] ?>" style="float:left;width:10px;height:10px;background-color:#<?= $rSqlItem['cor'] ?>;border:1px solid #<?= $rSqlItem['cor'] ?>;margin-top:4px;"></div>
                                                                    <div style="float:left;margin-left:5px;"><?=$rSqlItem['nome']?></div>
                                                                </div>
                                                                <? } ?>
                                                                ...
                                                                <input type="hidden" id="lista-categorias" value="<?=$lista_categorias?>" />
                                                    ...
                                                    <div>
                                                        <form name="form_categoria" method="post" action="<?=$link?><?=$_REQUEST['var1']?>/<?=$_REQUEST['var2']?>" target="_self" ENCTYPE="multipart/form-data">
                                                                ...
                                                                <div style="float:left;width:100%;" id="form_categoria">
                                                                    ...
                                                                    <div class="formSep">
                                                                        ...
                                                                        <select name="ordem" name="ordem_categoria" style="width:50px;">
                                                                            <?
                                                                            $nordem = mysql_num_rows(mysql_query("SELECT * FROM parceiro_sysagenda_categoria WHERE stat='1' ORDER BY ordem"));
                                                                            if($nordem==0) {
                                                                            ?>
                                                            ...
                                                            <div class="formSep" style="display:block;" id="btns-add">
                                                                <button type="button" onclick="salvar_categoria_parceiro_sysagenda();" style="margin-top:23px;" class="btn btn-primary">Adicionar</button>
                                                            </div>
                                                            
                                                            <div class="formSep" style="display:none;" id="btns-editar">
                                                                <button type="button" onclick="salvar_categoria_parceiro_sysagenda();" style="margin-top:23px;" class="btn btn-success">Salvar</button>
                                                                <button type="button" onclick="cancela_edita_categoria_parceiro_sysagenda('<?=$sysusu['id']?>');" style="margin-top:23px;" class="btn btn-warning">Cancelar</button>
                                                            </div>
                                                            ...
                                                            <div style="float:left;margin-top:10px;width:100%;padding-bottom:10px;border-top:1px dashed #CCCCCC;padding-top:10px;">
                                                                ...
                                                                <div id="lista_categoria_itens" style="width:100%;float:left;">
                                                                    ...
                                                                    <tbody>
                                                                        <?
                                                                        $qSqlCategoria = mysql_query("SELECT * FROM parceiro_sysagenda_categoria ORDER BY ordem");
                                                                        ...
                                                                        <tr id="lista_categoria_<?=$rSqlCategoria['id']?>">
                                                                            ...
                                                                            <td style="vertical-align:middle;" class="nolink">
                                                                                <div class="btn-group">
                                                                                <a href="javascript:void(0);" onclick="edita_parceiro_sysagenda_categoria('<?=$rSqlCategoria['id']?>','');" class="btn-mini ptip_se" title="Editar"><img src="<?=$link?>template/img/icones_novos/16/editar.png" /></a>
                                                                                <a href="javascript:void(0);" onClick="remover_parceiro_sysagenda_categoria('<?=$rSqlCategoria['id']?>','<?=$sysusu['id']?>');" class="btn-mini ptip_se" title="Remover"><img src="<?=$link?>template/img/icones_novos/16/remover-x.png" /></a>
																				<? if(trim($rSqlCategoria['stat'])=="1") { ?>
                                                                                    <a href="javascript:void(0);" onclick="muda_stat_parceiro_sysagenda_categoria('<?=$rSqlCategoria['id']?>','<?=$sysusu['id']?>','0');" class="btn-mini ptip_se" title="Despublicar"><img src="<?=$link?>template/img/icones_novos/16/stat-1.png" /></a>
                                                                                <? } else { ?>
                                                                                    <a href="javascript:void(0);" onclick="muda_stat_parceiro_sysagenda_categoria('<?=$rSqlCategoria['id']?>','<?=$sysusu['id']?>','1');" class="btn-mini ptip_se" title="Publicar"><img src="<?=$link?>template/img/icones_novos/16/stat-0.png" /></a>
                                                                                <? } ?>
                                                                                </div>
                                                                            </td>
                                                                        </tr>
            ...
            <script>filtra_parceiro_sysagenda('','<?=$sysusu['id']?>','<?=$_REQUEST['var1']?>','<?=$_REQUEST['var2']?>','','0');</script>
