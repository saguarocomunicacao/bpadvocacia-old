<?
	...
	if(trim($_POST['acaoForm'])=="add") {
		...
		echo"<script>window.open('".$link."".$_REQUEST['var1']."/suporte/','_self','')</script>";
	} else {
		if(trim($_POST['acaoForm'])=="editar") {
			...
			if(trim($_POST['idsysusu'])==""||trim($_POST['idsysusu'])==0) {
				...
				echo"<script>window.open('".$link."".$_REQUEST['var1']."/suporte/responder/".$item['idchamado']."','_self','')</script>";
			} else {
				include("mod/syssuporte/email-resposta.php");
				echo"<script>window.open('".$link."".$_REQUEST['var1']."/suporte/detalhes/".$item['idchamado']."','_self','')</script>";
			}
		} else {
		}
	}
